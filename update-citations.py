...
# As an example, the FieldTrip reference paper has the following identifiers
# PMID  = 21253357
# PMCID = PMC3021840
...
url = "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/elink.fcgi?dbfrom=pubmed&linkname=pubmed_pmc_refs&retmode=json&id=" + PMID
r = requests.get(url)
d = json.loads(r.text)
...
for pmcid in citedby:
    filename = os.path.join(MODALITY, pmcid + '.yml')
